/*
   ...
   on a ESP32 slave device (without debouncing) and output touch button, 
   hall, and temperature readings.

   Needs ESP32sensorsI2C.h module enabled in the slave's firmware_modules.h.
...
const uint8_t interruptPinSlave  = 26; // needs to be wired to interruptPinMaster
const uint8_t interruptPinMaster = 26; // wired to interruptPinSlave, needs to be a hardware interrupt pin (2 or 3 on Arduino Uno/Nano, any ESP32 pin)
...
ESP32sensorsI2C sensors(&wrapper); // ...that the pin interface needs to communicate with the slave
...
//  ###
  ...
  // First, make the slave send interrupts. The interrupt pin is shared by all slave modules
  ...
  sensors.enableInterrupts(touchPin, 40); // make slave send out interrupts for this pin at the interupt pin set above
  
  // make the master listen to the interrupt pin
...
void loop()
{
  if (interruptFlag) {
    ...
    if (triggerReason == interruptReason_ESP32sensorsTouch and triggerUnit == touchPinID) {
      ...
      Serial.print(" | temperature (probably useless) = "); Serial.println(sensors.temperatureRead());      
    } else {
  ...
  //  if (millis()-then > 1000) {
  //    Serial.print(digitalRead(interruptPinMaster));
  //    Serial.print("  "); 
